...
This section describes how we use the PINN used in our study. The Lorenz--1960 equations,
...
the correct initial state. To satisfy this constraint exactly,we construct a hard trial solution that contains the network output. The raw network output $\mathcal{N}(t_i;\theta)$ is not used as the final solution.
...
\begin{equation}
    ...
    \label{eq:lorenz-rhs}
\end{equation}
where $(c_x,c_y,c_z)=(-0.10,1.60,-0.75)$ for $k=2$ and $l=1$. PyTorch
...
mean-squared value.We sample $N_c=3000$ time points with a Latin-hypercube design on $[0,1]$.
